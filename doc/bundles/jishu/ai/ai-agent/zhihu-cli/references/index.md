---
okf_version: "0.2"
type: Reference
title: "参考与信源索引"
description: "Zhihu CLI 知识包参考层导航，含官方 API 参考、事实登记、核验报告与外部信源链接。"
tags: ["信源索引", "参考层", "F编号", "核验报告", "官方API"]
generated: 2026-09-04
verified: 2026-09-05
status: verified
stale_after: "2026-12-31"
sources:
  - "facts.md"
  - "verification.md"
  - "article-source.md"
  - "official-api-reference.md"
---

# 参考与信源

| 文档 | 内容 | 条目数 |
|------|------|--------|
| [official-api-reference.md](official-api-reference.md) | 官方 API 接口参考手册（5 大核心接口 + 用户数据 5 接口 + OAuth + MCP + 鉴权 + 错误码） | 11 接口 + 7 额度 + 5/6 错误码 |
| [article-source.md](article-source.md) | 文章信源事实登记（F-001~F-150） | 150 条事实 |
| [verification.md](verification.md) | P0 权威核验报告 + 勘误 + 时效边界 | 25 项核验，3 条勘误 |

## 外部信源

### 主信源

| 编号 | 来源 | 类型 | 信源距离 |
|------|------|------|----------|
| S1 | 腾讯云开发者社区 | 作者一手实测 | 一手 |
| S2 | 觉醒AI博客 | 作者一手实测 | 一手 |
| S3 | 知乎官方开放平台 (developer.zhihu.com) | 官方发布 | 官方 |
| S4 | 知乎问题页 | 社区讨论 | 社区 |
| S5 | 老狼知乎专栏 | 作者一手实测 | 一手 |
| S6 | 老狼知乎回答 | 作者一手实测 | 一手 |
| S7 | 知乎开发者官方文档 (developer.zhihu.com/docs) | 官方技术文档 | 官方（一手技术规格） |

### 官方资源

- 知乎数据开放平台：https://developer.zhihu.com
- 知乎开发者文档：https://developer.zhihu.com/docs

### 核验补充来源

- 威易网 2026-05-19 报道
- 什么值得买 2026-09-03 文章
- 华东子 2026-08-15 文章
- 今日头条技术媒体报道

---

```{toctree}
:hidden:

official-api-reference
article-source
verification
```
