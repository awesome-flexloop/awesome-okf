...
description: "Zleap-Agent Workspace 隔离机制与上下文组装：main/work 空间、数据库为唯一真源、when/notFor 路由提示、persona、toolIds；Kernel 经 switchWorkspace 路由；Context 稳定/半稳定/可变三块组装与缓存断点不变量。"
tags: ["zleap-agent"]
sources:
  - id: learning-source
    resource: "docs/knowledge/learning/00-agent-frameworks/zleap-agent-wiki/02-workspace-context.md"
