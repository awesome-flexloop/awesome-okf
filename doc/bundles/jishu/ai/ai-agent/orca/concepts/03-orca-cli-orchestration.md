...
description: "Orca CLI 命令面与多 Agent 编排机制详解：worktree/terminal/repo/automations/browser/linear/computer/orchestration 八大命令族、Run/Task/Dispatch/worker_done 核心编排概念、受监督工作流（task-create → worker-start → check --wait）与完整交接（f"
tags: ["orca", "agent-orchestration", "multi-agent"]
sources:
  - id: learning-source
    resource: "docs/knowledge/learning/03-orca-cli-orchestration.md"
