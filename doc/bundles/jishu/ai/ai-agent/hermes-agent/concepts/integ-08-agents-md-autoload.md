...
description: "Hermes 原生自动加载 AGENTS.md（启动 + 子目录渐进发现），但不会自动加载 .agents/ 目录；需通过路由地图、插件或 OKF 挂载让规范库被正确识别"
tags: ["hermes-agent", "integration"]
sources:
  - id: learning-source
    resource: "docs/knowledge/learning/00-agent-frameworks/hermes-agent-integration/08-agents-md-autoload.md"
