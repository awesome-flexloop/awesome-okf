...
description: "本章面向希望完全掌控安装过程的开发者、贡献者以及 CI/CD 环境，详细说明如何从 Git 仓库手动克隆并安装 Hermes Agent。内容涵盖仓库克隆、uv 与 venv+pip 两种 Python 环境方案、完整的依赖说明（核心依赖与可选 extras）、Node.js 前端依赖安装与构建、开发者模式、Playwright 浏览器安装、`hermes` 命令的 PATH 配置，以及 `set"
tags: ["hermes-agent", "installation"]
sources:
  - id: learning-source
    resource: "docs/knowledge/learning/00-agent-frameworks/hermes-agent-installation/04-install-manual.md"
