...
description: "本章详细说明如何使用 Docker 构建、部署和运维 Hermes Agent 容器。内容涵盖 Docker 镜像构建方法、`docker-compose.yml` 配置说明、卷挂载与权限配置、s6-overlay 进程管理机制、常用 `docker compose` 命令、环境变量传递方式、Dashboard 安全提示、镜像分层结构以及网络模式。所有信息均以项目源码中的 `Dockerfile`"
tags: ["hermes-agent", "installation"]
sources:
  - id: learning-source
    resource: "docs/knowledge/learning/00-agent-frameworks/hermes-agent-installation/05-install-docker.md"
