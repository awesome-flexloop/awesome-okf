...
description: "本章详细说明 Hermes Agent 的版本升级、配置备份、版本回滚、卸载流程以及数据目录结构。内容涵盖 `hermes update` 自动升级命令的完整工作流、手动升级步骤、Docker 镜像升级、升级前备份策略、版本回滚方法、`hermes uninstall` 卸载命令、跨平台手动卸载步骤、`~/.hermes` 数据目录说明，以及升级过程中的常见问题与解决方案。"
tags: ["hermes-agent", "installation"]
sources:
  - id: learning-source
    resource: "docs/knowledge/learning/00-agent-frameworks/hermes-agent-installation/09-upgrade-uninstall.md"
