...
description: "本章面向希望在 Android 设备上通过 Termux 运行 Hermes Agent 的用户，详细说明 Termux 环境的特殊限制、系统依赖安装、安装脚本的 Termux 专属行为、`[termux]` / `[termux-all]` extras 构成、不支持的功能清单、存储权限配置、后台保活方法、常见故障排查以及性能优化建议。所有内容均以项目源码中的 `scripts/install."
tags: ["hermes-agent", "installation"]
sources:
  - id: learning-source
    resource: "docs/knowledge/learning/00-agent-frameworks/hermes-agent-installation/10-termux.md"
