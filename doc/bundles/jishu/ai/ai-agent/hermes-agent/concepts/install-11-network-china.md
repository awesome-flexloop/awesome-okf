...
description: "本章面向位于中国大陆网络环境下的用户，汇总 Hermes Agent 安装与运行过程中涉及的所有境外资源的镜像加速方案。Hermes Agent 的安装链路需要访问 GitHub（源码克隆）、PyPI（Python 包）、npm registry（Node 包）、nodejs.org（Node 二进制）、Playwright CDN（浏览器二进制）、Docker Hub（容器镜像）、Hugging"
tags: ["hermes-agent", "installation"]
sources:
  - id: learning-source
    resource: "docs/knowledge/learning/00-agent-frameworks/hermes-agent-installation/11-network-china.md"
