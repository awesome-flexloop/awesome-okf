---
type: Reference
title: "Oracle 博文信源与事实登记"
description: "微信公众号「Leon学AI」Oracle 玩法博文的元信息、信源距离分级与 F-001~F-041 事实登记（双份登记）"
tags: [Oracle, 信源登记, 事实底稿, Codex, ChatGPT, 博文转化]
sources:
  - id: blog
...
| 项 | 内容 |
|----|------|
| 标题 | 《太炸裂了！这是哪个大佬发现的 CodeX这个神仙用法，居然能将gpt-plus发挥到极致！》 |
| 作者/公众号 | 「Leon学AI」（无独立作者署名，IP 属地广东），原创 |
| 发布时间 | 2026-08-30 21:45 |
| URL | https://mp.weixin.qq.com/s/_J1BzyqyoNWuj_998DLo1g |
| 话题标签 | #codex教程 |
| 篇幅与形态 | 正文 1684 字；无信息性图片；GitHub 地址以纯文本出现（非超链接）；无"阅读原文"、无广告/转载声明；有赞赏入口 |
| 内容类型 | 开源工具玩法推荐 + 简明安装接入指引（转述种草性质，作者未声明一手实测，行文为"有人开源做好了"） |
| 涉及项目 | https://github.com/steipete/oracle（Peter Steinberger，MIT） |

## 信源距离分级

| 信源 | 距离 | 用途 |
|------|------|------|
| GitHub 官方仓库（main 分支 docs/install.md、docs/browser-mode.md、docs/agents.md、skills/oracle/SKILL.md、README） | ① 官方发布 | 命令、参数、平台要求、接入步骤、行为机制的裁决依据 |
| npm 官方包页（@steipete/oracle） | ① 官方分发渠道 | 包名、版本（0.20.2）、安装方式、引擎矩阵、Node 要求旁证 |
| 官网 askoracle.sh | ① 官方发布 | 项目定位旁证（未逐页抓取） |
| everydev.ai 工具索引 | ④ 第三方综述 | 仅用于项目创建时间（2025-11）与 star 数等弱事实，已标 ⚠️ 单源，不进正文结论 |
| 微信公众号「Leon学AI」原文 | ③/④ 第三方推荐（非一手实测） | 玩法叙事、分工比喻、使用体验与"额度"主张的来源 |

> **信源距离预判结论**：博文非厂商约稿（无广告标识、Oracle 作者 steipete 与公众号无关联证据），也不含"提效倍数/节省工时"类成效数字——唯一的效用主张"Codex 额度更耐用"是**定性、无数据的作者体验**（F-008），且博文自行澄清"不是额度互换"（F-019）。该主张按 📌 作者观点处理，不按成效数字做 P0 溯源；真正需要 P0 核验的是全部可执行命令与项目身份，共 10 项，全部对官方源通过。

## 博文叙事结构（原文脉络）

1. 痛点开场：Codex 与 ChatGPT Plus"两边各用各的"
2. 四步串联：Codex 读项目 → 整理上下文 → 网页 ChatGPT 复杂推理 → Codex 回本地执行
3. 价值主张：把吃额度的重推理分流到已付费的网页端（"GPT 当大脑，Codex 当双手"）
4. 工具登场：开源项目 Oracle（steipete），Browser Mode 复用已登录会话
5. 操作三段：安装（brew/npm）→ Browser Mode 首次登录 → clone 仓库装 Codex skill + 配 AGENTS.md
6. 边界自澄清与推荐人群

## 事实登记（F-001~F-041）

> F-001~F-021 为博文事实（其中 F-002、F-004、F-006、F-007、F-008、F-011 评价部分、F-020 为作者观点）；F-022~F-041 为官方源核验补充。
> 核验状态：✅ 官方一致 ｜ ⚠️ 口径差异/单源 ｜ 📌 作者观点（非客观事实）。
> 本登记与 spec `facts.md` 为双份登记，两集合须一致（V 阶段正则比对）。
...
| F-033 | 官方 Golden path：选最小且包含真相的文件集 → `--dry-run` + `--files-report` 预览解析文件与 token 估算 → browser 跑 Pro 模型 → detach/超时时 reattach 已存会话而非重复提交 | ✅ 仓库 skills/oracle/SKILL.md（main） |
...
> Codex：连接电脑、读取项目、修改代码、执行和验证。
> ChatGPT Plus：复杂分析、重推理、方案规划。
>
> 项目叫 Oracle，作者是 steipete。GitHub：https://github.com/steipete/oracle
>
> 它做的事情很直接：把 Prompt 和选中的本地文件整理成 Context，再交给另一个模型分析，最后把结果拿回来。重点是，它支持 Browser Mode，可以使用已经登录的 ChatGPT 网页会话。
>
> 它当然不是把 Plus 额度直接变成 Codex Pro 额度，两边也不是额度互换。真正厉害的是：你把已经付费的 ChatGPT 网页端推理能力，正式接进了 Coding Agent 工作流。
