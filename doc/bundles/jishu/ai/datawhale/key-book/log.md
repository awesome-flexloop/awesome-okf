...
- **V 阶段**：
  - 章节标题与 catalog（README 目录表 + VitePress `config.mjs` 导航）对齐。发现两处源项目自身不一致：第 1 章源文件 H1 为"预备定理"但 catalog 为"预备知识"；第 3 章源文件 H1 为"复杂性分析"但 catalog 为"复杂度"。知识包统一采用 catalog 标题，并在参考文件中标注源 H1 差异。
