...
sources:
  - id: learning-source
    resource: "docs/knowledge/learning/05-ai-multimodal-content/mainecoon-wiki/05-critique-and-methodology.md"
...
> 本节为 [analysis-report.md](analysis-report.md) 原子化拆分的第六部分，包含批判性思考、SpecWeave 关联分析与七概念方法论（F+V）深度审视。方法论模式已萃取为独立模式文档（trilemma-architectural-resolution.md「三角困境→架构级解决框架」与 honest-limitation-acknowledgment.md「诚实承认局限性信任构建策略」）。
...
> 以下五大方法论中,方法论一(三角困境)与方法论四(诚实承认局限性)已萃取为完整模式文档,详见 trilemma-architectural-resolution.md（三角困境→架构级解决框架）与 honest-limitation-acknowledgment.md（诚实承认局限性信任构建策略）。
...
- **完整模式文档**: trilemma-architectural-resolution.md（三角困境→架构级解决框架，SpecWeave 复盘模式库）
...
- **当前 SpecWeave**:.agents/capability-boundaries.md（能力边界声明）已经采用“职责边界 + 禁止事项”的诚实表述
...
- **完整模式文档**: honest-limitation-acknowledgment.md（诚实承认局限性信任构建策略，SpecWeave 复盘模式库）
