...
本目录登记 podman-py 知识包所有内容据以派生的源码+AGENTS信源。所有内容文档（11 概念+4 示例+4 信源登记）的 frontmatter `sources` 字段均锚定本目录的条目。R3（2026-09-14）起新增内容锚定 vendor 稳定信源（git submodule pin commit）。本目录合计 4 篇：R1/R2 的 20+20+30=**70 条编号锚点 + 1 调用链图**，加 R3 的 **39 文件结构地图 + 核心符号索引 + 资源端点差异表**，保证"每一个技术结论 → 一条可 grep 的源码锚点 → 一段可读原文"一一对应，可对抗审查（V 阶段）。
...
* [vendor 全量源码信源登记（R3，commit 5dd81b4）](source-code-map.md) — 版本固定（v5.8.0-9-g5dd81b4 / 39 文件 6078 行：api 11/1209 + domain 22/4455 + errors 2/197 + 顶层 4/217；测试 unit 26 + integration 11）；四层结构地图（包顶层/api/domain/errors 逐文件行数+职责）；R3 新覆盖模块核心符号索引（UDS 四件套、帧消费器、双格式配置、9 资源模型）；资源端点与身份差异速查表（id 键/get 后缀/prune 错误键/SpaceReclaimed/list 404 语义）；9 条版本/行为门槛。
