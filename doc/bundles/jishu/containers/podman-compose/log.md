...
- `examples/00-getting-started.md`：零基础入门教程（约 15 分钟）——前置条件与安装、单服务起步、数据库+命名卷持久化、`${VAR:-default}` 参数化、常用命令速查表、本地 Dockerfile 构建、5 个新手坑（数据保留/服务名互连/列表追加合并/depends_on 边界/rootless UID）、下一步学习路径；内容全部基于本束已验证的概念与示例文档，交叉链接 10 篇。

### 更新

- `examples/index.md`：示例清单置顶 00，学习路径新增"零起点"段，toctree 置顶 00-getting-started
- 根 `index.md`：Bundle 结构树补充 00，实战示例导航新增入门教程入口（置顶）
