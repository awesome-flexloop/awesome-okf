...
tags: [git, baidu-netdisk, sync, workflow, initialization]
generated: { by: "process:learning-bundles-migration", at: "2026-09-02T00:00:00Z" }
status: draft
stale_after: 2027-09-02
sources:
  - id: learning-source
    resource: SpecWeave docs/knowledge/learning/08-systems-infrastructure/git-baidu-sync/03-repo-init-workflow.md
    title: original
---
...
```powershell
# 方式1：使用正斜杠（最兼容）
git remote add baidu "<同步盘>/git-sync/repos/my-project.git"

# 方式2：file:// 协议 + 正斜杠
git remote add baidu "file:///<同步盘>/git-sync/repos/my-project.git"
...
| `sync_root` | 本设备上网盘同步根路径 | `<同步盘>/git-sync` |
...
| 平台 | 典型路径 |
|------|---------|
| Windows | `<同步盘>\git-sync`、`C:\Users\<用户名>\BaiduSync\git-sync` |
...
$env:GIT_SYNC_ROOT = "<同步盘>\git-sync"
```
