...
sources:
  - id: learning-source
    resource: SpecWeave docs/knowledge/learning/08-systems-infrastructure/git-baidu-sync/10-troubleshooting.md
    title: original
---
# Git 百度网盘同步 - 故障排查手册
...
.\git-diag.ps1 -RepoPath D:\projects\myrepo -SyncRoot <同步盘>\git-sync
...
push时输出：
```
fatal: '<同步盘>\git-sync\repos\myrepo.git' does not appear to be a git repository
...
git remote -v
# 预期输出类似：
# baidu  <同步盘>\git-sync\repos\myrepo.git (fetch)
# baidu  <同步盘>\git-sync\repos\myrepo.git (push)

# 2. 检查路径是否存在
# PowerShell:
Test-Path "<同步盘>\git-sync\repos\myrepo.git"
# 预期输出：True

# 3. 检查路径是否是有效的git裸仓库
Test-Path "<同步盘>\git-sync\repos\myrepo.git\HEAD"
Test-Path "<同步盘>\git-sync\repos\myrepo.git\objects"
# 预期都为True
```
...
2. **克隆仓库（推荐方式，用封装脚本）**：
   ```powershell
   # 进入脚本目录
   cd <local-workspace>.agents\scripts\git-baidu-sync
   
   # 克隆单个仓库
   .\clone-repo.ps1 -RepoName <repo名> -SyncRoot <SyncRoot路径> -TargetPath <本地工作目录>
   
   # 示例：
   .\clone-repo.ps1 -RepoName myproject -SyncRoot <同步盘>\git-sync -TargetPath D:\projects\myproject
   ```
