...
> **⚠️ 数据来源说明**：以下数据引自微软官方博客（二级来源），由微软工程师在同一台机器上分别运行 WSL Containers 和 Docker Desktop 测得。
> 尚未发现第三方独立复现数据（三级来源）。不同硬件配置和实际工作负载可能与下表存在差异，建议在目标环境中自行验证。
...
| # | 攻击点 | 攻击路径 | 严重度 | 回应 |
|---|--------|---------|--------|------|
| V-1.1 | **性能数据来源单一** | 基准测试表（6.1节）仅引用微软官方博客数据，无第三方独立复现。若微软有意淡化对比，数字可能被选择性呈现。 | P2 | ✅ 已在6.1节前增加数据来源说明block：明确标注为微软官方博客二级来源，提示未发现第三方独立复现数据，建议读者在目标环境自行验证。 |
