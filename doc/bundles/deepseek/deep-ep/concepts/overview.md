...
|--------------|-----------------|
| [DeepGEMM](/deepseek/deep-gemm) | MoE 分组 GEMM 内核，在 dispatch 之后执行专家计算，combine 之前完成 |
| [LPLB](/deepseek/lplb) | 专家负载均衡器，输出 `topk_idx` 路由决策给 dispatch |
| [DualPipe](/deepseek/dual-pipe) | 双向流水线并行，与 EP 组合使用，ElasticBuffer 提供 PP send/recv 原语 |
