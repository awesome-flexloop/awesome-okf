...
| 组件 | 路径 | 关系 |
|---|---|---|
| DeepEP | [/deepseek/deep-ep/](/deepseek/deep-ep/) | EP 通信库，提供高效 all-to-all 和 NVSHMEM 通信原语 |
| DeepGEMM | [/deepseek/deep-gemm/](/deepseek/deep-gemm/) | MoE grouped GEMM 计算内核，LPLB 均衡直接影响其尾延迟 |
...
- [总览](/deepseek/lplb/concepts/overview) — LPLB 是什么、解决什么问题、核心机制
- [负载均衡问题建模](/deepseek/lplb/concepts/load-balancing-problem) — MoE 负载不均衡的数学形式化与 LP 模型
- [LP 求解器设计](/deepseek/lplb/concepts/lp-solver-design) — GPU 端内点法、cuSolverDx 集成、JIT 编译架构
- [拓扑感知路由](/deepseek/lplb/concepts/topology-aware-routing) — 拓扑矩阵语义、预定义拓扑、副本组布局
- [EPLB 集成](/deepseek/lplb/concepts/eplb-integration) — 静态重平衡与动态求解的分层协作

### API 参考

- [API 参考](/deepseek/lplb/references/api) — Planner 类、EPLB 函数、CompiledSolver 接口
- [LP 求解器](/deepseek/lplb/references/lp-solver) — CUDA 内核、JIT 编译流程、NVSHMEM 通信
- [拓扑配置](/deepseek/lplb/references/topology) — r2o 矩阵约束、内置拓扑、副本组机制

### 使用示例

- [基础专家复制规划](/deepseek/lplb/examples/basic-planning) — 创建 Planner、分步执行、验证均衡效果
- [拓扑感知配置](/deepseek/lplb/examples/topology-config) — 各种拓扑使用、自定义拓扑、DeepEP 集成
