...
- [核心SDK与三层架构](/datawhale/deepagents/concepts/core-sdk) 提供 Agent 运行时基础。
- [Code终端编码Agent](/datawhale/deepagents/concepts/code-module) 是 Talon 的 Agent 运行时依赖（`deepagents-code>=0.1.30`）。
- [Monorepo 架构](/datawhale/deepagents/concepts/monorepo-architecture) 描述了 talon 包在仓库中的位置。
