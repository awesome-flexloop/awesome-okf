...
- [核心SDK与三层架构](/datawhale/deepagents/concepts/core-sdk) 是 dcode 构建于其上的 Agent 框架。
- [ACP协议集成](/datawhale/deepagents/concepts/acp-protocol) 允许 dcode 通过 `--acp` 标志嵌入编辑器。
- [Evals评估套件](/datawhale/deepagents/concepts/evals-suite) 对 dcode 的编码能力进行基准测试。
- [CLI部署工具](/datawhale/deepagents/concepts/cli-toolchain) 是从 code 拆分出的独立部署工具。
- [Talon运行时宿主](/datawhale/deepagents/concepts/talon-runtime) 依赖 deepagents-code 作为其 Agent 运行时。
