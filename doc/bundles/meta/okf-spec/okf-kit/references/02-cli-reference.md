...
description: "okf-kit 使用 Python 标准库 argparse 提供 10 个 CLI 子命令，分为构建维护（build/validate/zip/sync）、知识消费（list/get/chat/visualize）和服务暴露（serve-"
tags: [okf-kit, cli, command-reference, build, sync, chat, mcp, serve, argparse]
sources:
  - id: learning-source
    resource: "SpecWeave docs/knowledge/learning/03-agent-platforms-tools/okf-kit-wiki/references/02-cli-reference.md"
