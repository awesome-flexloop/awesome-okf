...
description: "OKF（Open Knowledge Format v0.1）将知识表示为"目录中带 YAML frontmatter 的 Markdown 文件"，每个非保留 Markdown 文件必须包含 `type` 字段标识概念类型，每个目录有 `"
tags: [okf-kit, okf, format, frontmatter, bundle, state.json, index.md]
sources:
  - id: learning-source
    resource: "SpecWeave docs/knowledge/learning/03-agent-platforms-tools/okf-kit-wiki/concepts/03-okf-format.md"
