---
type: Concept
title: Knowledge Catalog 参考 Agent 实现
description: knowledge-catalog 参考 Agent 源码级解析——两阶段工作流架构、enrich 子命令参数、核心工具模块（bundle/source/web/context）、Python 实现细节、单概念迭代开发方法与 GCP 凭证配置。
tags: [okf, knowledge-catalog, reference-agent, enrich, python, gcp]
generated: { by: process:learning-bundles-merge, at: 2026-09-02T00:00:00Z }
status: draft
stale_after: 2027-09-02
sources:
  - id: src1
    resource: SpecWeave docs/knowledge/learning/01-agent-protocols-interfaces/knowledge-catalog-wiki/03-reference-agent.md
    title: Knowledge Catalog Wiki 参考 Agent 章（01 分类）
  - id: src2
    resource: SpecWeave docs/knowledge/learning/07-vendor-product-learning/google-cloud/knowledge-catalog-wiki/02-reference-agent.md
    title: Knowledge Catalog Wiki 参考智能体章（07 分类）
---
# 03 参考Agent实现原理与运行指南
...
> - 首次使用建议先完成 okf-wiki 5分钟快速入门 的手工实操，建立OKF Bundle的感性认识后再阅读本章。
...
仓库中包含4个现成的可浏览bundle，由参考智能体生成并提交到`bundles/`：

| Bundle | 数据集 | 特点 | viz.html |
|--------|--------|------|----------|
| `bundles/ga4/` | GA4电商数据集（Google Merch Store） | 标准GA4 BigQuery Export schema，events_日分片表 | [viz.html](file:///<local-workspace>vendor/knowledge-catalog/okf/bundles/ga4/viz.html) |
| `bundles/stackoverflow/` | Stack Overflow公开数据集 | 多表（badges/comments/posts/users/votes等），多对多关系，跨表连接文档丰富 | [viz.html](file:///<local-workspace>vendor/knowledge-catalog/okf/bundles/stackoverflow/viz.html) |
| `bundles/crypto_bitcoin/` | Bitcoin区块链数据集 | blocks/transactions/inputs/outputs四表，外键关系在prose中表达 | [viz.html](file:///<local-workspace>vendor/knowledge-catalog/okf/bundles/crypto_bitcoin/viz.html) |
| `bundles/acme_retail/` | Acme Retail示例 | 展示认证计算(Attested Computation)完整示例：metrics/policies/computations/attesters/skills | [viz.html](file:///<local-workspace>vendor/knowledge-catalog/okf/bundles/acme_retail/viz.html) |
