...
| 文档 | 内容 |
|---|---|
| 概述 | 架构总览、功能模块、支持矩阵、包结构、快速开始 |
| MLA 解码算法 | 低秩 KV 压缩原理、解码计算流程、online softmax、MQA/GQA 支持 |
| SplitKV 长序列技术 | KV 分块并行、调度元数据、两阶段执行、Ring Buffer 流水线 |
| FP8 KV Cache 量化 | FP8 E4M3 量化方案、V32/MODEL1 对比、反量化流程、精度设计 |
| Hopper/Blackwell 内核 | WGMMA+TMA+DSM vs tmem+UTCMMA+UTCCP 架构差异与优化 |

### 📗 API 参考 [references/](references/index.md)

| 文档 | 内容 |
|---|---|
| Python API | `flash_mla_with_kvcache`、`get_mla_metadata`、`flash_mla_sparse_fwd`、`flash_attn_varlen_func` 完整签名 |
| 内核架构 | SM90/SM100 内核配置、MMA 指令、Shared Memory 布局、编译配置 |
| KV Cache 布局 | V32 (656B) / MODEL1 (576B) FP8 KV cache 内存布局、分页结构、索引格式 |

### 📙 代码示例 [examples/](examples/index.md)

| 示例 | 内容 |
|---|---|
| 基础解码 | BF16 dense 和 FP8 sparse MLA 解码的完整使用示例 |
| 性能基准测试 | Benchmark 脚本用法、自定义性能测试、正确性验证 |
...
| 项目 | 关系 |
|---|---|
| [DeepGEMM](../deep-gemm/index.md) | DeepSeek 高性能 JIT GEMM 核函数库，为 LLM 训练/推理提供矩阵乘法能力，与 FlashMLA 在推理 pipeline 中协同使用——FlashMLA 负责注意力计算，DeepGEMM 负责 MLP/MoE 线性层 |
| [TileLang Kernels](../tile-kernels/index.md) | TileLang 编写的核函数库，提供更高抽象层级的算子开发方式，作为 FlashMLA CUDA C++ 手写核函数的补充 |
| [DeepEP](../deep-ep/index.md) | DeepSeek 专家并行通信库，提供 all-to-all 通信原语，用于 MoE 模型的分布式推理与训练 |
