...
本章分析在线学习中算法的累积损失与事后最优策略的差距，是[遗憾界](/ai/datawhale/key-book/concepts/regret-bound)概念的主体章节，标志着从批量 i.i.d. 范式到在线非平稳范式的转换。
...
本章完成了全书最根本的范式转换：从 i.i.d. 批量统计学习到无分布假设的在线博弈学习。遗憾界不仅是在线学习的性能度量，更在大模型时代的分布偏移、持续学习、对抗环境中具有现实意义。$O(\sqrt{T})$ 全信息与 $O(T^{3/4})$ 拔河信息的差距，精确量化了反馈信息结构对学习性能的决定作用。参见 [UCB 多臂赌博机案例](/ai/datawhale/key-book/examples/ucb-bandit)。

## 参见

- [第 5 章：稳定性](/ai/datawhale/key-book/references/chapter5)：在线学习的可塑性与离线稳定性的张力
- [第 7 章：收敛率](/ai/datawhale/key-book/references/chapter7)：离线 $O(1/\sqrt{T})$ 与在线 $O(\sqrt{T})$ 的数学同源性
