...
| 模块 | 核心抽象 | 说明 |
|---|---|---|
| [Runnable](/langchain-ai/langchainjs/concepts/runnable-interface) | `Runnable` 类 | 一切组件的统一执行接口 |
| [Messages](/langchain-ai/langchainjs/concepts/message-system) | `BaseMessage` 类族 | 对话消息类型系统 |
| [Tools](/langchain-ai/langchainjs/concepts/tool-definition) | `StructuredTool` 类 | 工具定义与调用 |
| [Prompts](/langchain-ai/langchainjs/concepts/prompt-templates) | `BasePromptTemplate` 类 | 提示模板 |
...
| 模块 | 核心 API | 说明 |
|---|---|---|
| [ReactAgent](/langchain-ai/langchainjs/concepts/react-agent) | `createAgent()` | 生产级 ReAct Agent |
| [Middleware](/langchain-ai/langchainjs/concepts/middleware) | `createMiddleware()` | Agent 横切扩展系统 |
...
所有核心组件——LLM、Chat Model、Prompt Template、Output Parser、Tool、Retriever——都继承自 `Runnable`。这意味着它们共享同一套执行接口（`invoke`/`batch`/`stream`）和组合子（`pipe`/`withRetry`/`withFallbacks`）。详见 [Runnable 接口](/langchain-ai/langchainjs/concepts/runnable-interface)。
...
- [Runnable 接口](/langchain-ai/langchainjs/concepts/runnable-interface) — 统一执行抽象与组合子
- [消息系统](/langchain-ai/langchainjs/concepts/message-system) — Message 类型层次与 tool_call
- [工具定义](/langchain-ai/langchainjs/concepts/tool-definition) — StructuredTool 与 tool 工厂
- [提示模板](/langchain-ai/langchainjs/concepts/prompt-templates) — PromptTemplate 与 ChatPromptTemplate
- [ReAct Agent](/langchain-ai/langchainjs/concepts/react-agent) — createAgent 与图编排
- [Middleware](/langchain-ai/langchainjs/concepts/middleware) — Agent 横切扩展
- [Document 与 Embedding](/langchain-ai/langchainjs/concepts/document-embedding) — 文档与向量化

### API 参考

- [Runnable 核心 API](/langchain-ai/langchainjs/references/core-runnable)
- [Message 与 Tool API](/langchain-ai/langchainjs/references/messages-tools)
- [Agent 与 Middleware API](/langchain-ai/langchainjs/references/agents-middleware)

### 使用示例

- [构建 LCEL 链](/langchain-ai/langchainjs/examples/lcel-chain) — Prompt → Model → Parser 管道
- [创建 ReAct Agent](/langchain-ai/langchainjs/examples/react-agent) — 工具调用 Agent 完整示例
