...
详见 [CLI 命令体系](/langchain-ai/langsmith-cli/concepts/cli-commands)。
...
认证采用四级优先级链：flag → 环境变量 → profile 配置 → 默认值。支持 API Key 和 OAuth 2.0 设备码流两种方式，配置存储在 `~/.langsmith/config.json`（0600 权限）。详见 [API 客户端架构](/langchain-ai/langsmith-cli/concepts/api-client)。
...
详见 [基础使用示例](/langchain-ai/langsmith-cli/examples/basic-usage) 和 [命令参考](/langchain-ai/langsmith-cli/references/commands)。
...
- [CLI 命令体系](/langchain-ai/langsmith-cli/concepts/cli-commands) — 命令树结构、过滤器系统、输出模式
- [API 客户端架构](/langchain-ai/langsmith-cli/concepts/api-client) — SDK 封装、v1/v2 适配、认证体系
- [命令参考](/langchain-ai/langsmith-cli/references/commands) — 全部命令与标志详解
