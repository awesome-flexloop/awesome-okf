...
- [Pregel 引擎](/langchain-ai/langgraph/concepts/pregel-engine) — 流在超步中的产出时机
- [消息图](/langchain-ai/langgraph/concepts/message-graph) — messages 流模式与 token 输出
- [状态图](/langchain-ai/langgraph/concepts/state-graph) — stream_mode 参数传递
- [API 参考](/langchain-ai/langgraph/references/graph-state) — stream 方法签名
