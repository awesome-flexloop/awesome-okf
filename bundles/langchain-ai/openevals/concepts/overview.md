...
所有评测器遵循统一协议：接收 `{ inputs, outputs, referenceOutputs, ... }`，返回 `EvaluatorResult`（含 `key`、`score`、`comment`、`metadata`）。详见 [评测器协议洞察](/langchain-ai/openevals/spec/insights)。
...
详见 [精确评测器](/langchain-ai/openevals/concepts/exact-evaluators)。
...
详见 [LLM-as-Judge](/langchain-ai/openevals/concepts/llm-as-judge)。
...
主要差异：Python 提供同步/异步双版本，JS 统一为 async；Python 支持 Pydantic/TypedDict schema，JS 支持 Zod schema。详见 [双语言架构洞察](/langchain-ai/openevals/spec/insights)。
...
详见 [基础评测示例](/langchain-ai/openevals/examples/basic-evaluation) 和 [API 参考](/langchain-ai/openevals/references/api)。

## 进一步阅读

- [精确评测器](/langchain-ai/openevals/concepts/exact-evaluators) — exactMatch、Levenshtein、嵌入相似度、JSON 匹配的实现机制
- [LLM-as-Judge](/langchain-ai/openevals/concepts/llm-as-judge) — 评分模式、prompt 格式化、judge 后端、结构化输出
- [API 参考](/langchain-ai/openevals/references/api) — 全部公共函数签名与参数说明
