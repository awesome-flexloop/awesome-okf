...
langchain-core 围绕一个统一协议——[Runnable 协议](/langchain-ai/langchain/concepts/runnable-protocol)——构建。所有组件（提示词、模型、输出解析器、检索器、工具）都是 `Runnable`，因此都自动具备：
...
这种"一切皆 Runnable"的设计使得组件可以任意组合，且组合后的链自动继承全部执行模式。详见 [Runnable 协议](/langchain-ai/langchain/concepts/runnable-protocol)。
...
- **消息体系**：`BaseMessage` 派生 `HumanMessage`、`SystemMessage`、`AIMessage`（含 `tool_calls`、`usage_metadata`）、`ToolMessage`（通过 `tool_call_id` 关联结果）。所有消息有对应的 `*Chunk` 变体支持流式拼接。`content_blocks` 属性将 provider 特定内容懒解析为统一的 `ContentBlock` 联合类型。详见 [消息类型](/langchain-ai/langchain/concepts/message-types)。
...
| 组件 | 基类 | 核心方法 | 概念文档 |
|---|---|---|---|
| 提示词 | `BasePromptTemplate` | `format`/`format_prompt` | [提示词系统](/langchain-ai/langchain/concepts/prompt-system) |
| 聊天模型 | `BaseChatModel` | `_generate`/`invoke`/`bind_tools` | [聊天模型](/langchain-ai/langchain/concepts/chat-model) |
| 工具 | `BaseTool` | `_run`/`invoke` | [工具抽象](/langchain-ai/langchain/concepts/tool-abstraction) |
| 输出解析 | `BaseOutputParser` | `parse`/`parse_result` | [输出解析器](/langchain-ai/langchain/concepts/output-parser) |
| 检索器 | `BaseRetriever` | `_get_relevant_documents` | [检索器与向量库](/langchain-ai/langchain/concepts/retriever-vectorstore) |
| 向量库 | `VectorStore` | `add_texts`/`similarity_search` | [检索器与向量库](/langchain-ai/langchain/concepts/retriever-vectorstore) |
| 嵌入 | `Embeddings` | `embed_documents`/`embed_query` | [检索器与向量库](/langchain-ai/langchain/concepts/retriever-vectorstore) |
...
- **Callbacks**：`BaseCallbackHandler` 通过多 Mixin 组合定义 `on_<component>_start/end/error` 生命周期方法；`CallbackManager`/`AsyncCallbackManager` 两棵独立的树管理 handler 传播。详见 [回调系统](/langchain-ai/langchain/concepts/callback-system)。
...
- [Runnable 协议](/langchain-ai/langchain/concepts/runnable-protocol) —— 执行接口、组合原语、配置传播
- [消息类型](/langchain-ai/langchain/concepts/message-types) —— BaseMessage 体系、ContentBlock、ToolCall
- [提示词系统](/langchain-ai/langchain/concepts/prompt-system) —— PromptTemplate、ChatPromptTemplate
- [聊天模型](/langchain-ai/langchain/concepts/chat-model) —— BaseChatModel、bind_tools、结构化输出
- [工具抽象](/langchain-ai/langchain/concepts/tool-abstraction) —— BaseTool、@tool、StructuredTool
- [回调系统](/langchain-ai/langchain/concepts/callback-system) —— Handler、Manager、Tracer
- [检索器与向量库](/langchain-ai/langchain/concepts/retriever-vectorstore) —— BaseRetriever、VectorStore、Embeddings
- [输出解析器](/langchain-ai/langchain/concepts/output-parser) —— BaseOutputParser、parse_result
