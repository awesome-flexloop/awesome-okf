---
type: reference
title: "myst-theme 项目结构与样式系统源�?
description: "monorepo 结构、styles/index.js Tailwind 配置、CSS 变量系统�?CSS 模块"
source_path: "external/libs/ai/jupyter-book/myst-theme/"
key_exports:
  - styles/index.js（Tailwind 配置入口�?  - CSS 模块（typography/code-highlight/proof/grid/jupyter 等）
  - CSS Variables�?-myst-color-*�?facts: [F-001, F-002, F-004, F-017, F-018, F-019, F-020, F-021, F-022, F-023, F-024]
...
```
myst-theme/
├── packages/           # 可复�?React 组件库（14 个子包）
�?  ├── myst-to-react/  # MDAST→React 渲染�?�?  ├── providers/      # React Context Provider
�?  ├── site/           # 站点级组件（Remix�?�?  ├── frontmatter/    # Frontmatter 渲染
�?  ├── jupyter/        # Jupyter/thebe 集成
�?  ├── diagrams/       # 图表（Mermaid�?�?  ├── icons/          # 图标
�?  ├── search/         # 搜索核心
�?  ├── search-minisearch/  # MiniSearch 实现
�?  ├── common/         # 公共类型和工�?�?  ├── landing-pages/  # 着陆页组件
...
构建工具链：
- **Turbo**：任务编排（dev/build/compile/lint/test�?- **pnpm workspaces**：包管理
- **Remix**：主题应用的 SSR 框架
- **Tailwind CSS**：样式框�?- **@tailwindcss/typography**：排版插�?
...
同时包含 `node_modules/` �?`../../packages/` 路径，确�?pnpm �?submodule 两种使用方式都能正确扫描�?
### 颜色系统

�?50 �?CSS 自定义属性，按命名空间分组：

**链接�?*�?- `--myst-color-link`、`--myst-color-link-hover`、`--myst-color-link-underline`

**主色/焦点**�?- `--myst-color-primary`、`--myst-color-primary-hover`
- `--myst-color-focus-ring`、`--myst-color-focus-outline`
- `--myst-color-active`、`--myst-color-active-bg`、`--myst-color-active-surface`

**中性色**�?- `--myst-color-bg`、`--myst-color-bg-secondary`（背景）
...
**语义色（admonition/proof 等）**�?- `--myst-color-info/info-bg/info-text`
...
### gridColumn 快捷�?
映射到命名网格线�?- `col-screen`、`col-screen-inset`、`col-page`、`col-page-inset`
...
覆盖 @tailwindcss/typography �?`--tw-prose-*` CSS 变量指向 myst CSS 变量�?- `--tw-prose-links` �?`var(--myst-color-link)`
...
包含动态生成的类名防止 Tailwind JIT purge 误删�?- 所�?col-* 网格�?- col-span-* �?row-span-*
- shaded/framed/shaded-children/rounded-children 工具�?- sphinx-* 类（Sphinx 文档兼容�?
## CSS 模块
...
| backmatter.css | 后页/参考文�?|
| ansi.css | ANSI 终端颜色 |
| sphinx.css | Sphinx �?签名样式 |
| text-spacers.css | 文本间距工具�?|
