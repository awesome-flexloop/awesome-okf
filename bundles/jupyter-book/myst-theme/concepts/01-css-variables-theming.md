...
myst-theme 的颜色系统完全建立在 CSS 自定义属性（CSS Variables/Custom Properties）之上。所有颜色值都通过 `--myst-color-*` 命名空间的变量定义，组件中使�?Tailwind 类名间接引用这些变量�?
```
CSS 变量定义（styles/*.css�?    �?Tailwind themeExtensions 映射（styles/index.js�?    �?组件中使�?Tailwind 类名（bg-myst-info-bg、text-myst-link�?    �?最终渲染为 CSS 变量引用
```
...
| 类别 | 变量示例 | 用�?|
|------|---------|------|
| 链接 | `--myst-color-link`, `--myst-color-link-hover` | 超链接颜�?|
| 主色 | `--myst-color-primary`, `--myst-color-primary-hover` | 按钮、主操作 |
| 焦点 | `--myst-color-focus-ring`, `--myst-color-focus-outline` | 键盘焦点指示�?|
| 激�?| `--myst-color-active`, `--myst-color-active-bg` | 活动状态（当前标签页等�?|
...
| 表面 | `--myst-color-surface`, `--myst-color-surface-hover` | 卡片、面板表�?|
| 文本 | `--myst-color-text`, `--myst-color-text-secondary/tertiary` | 正文、次要、辅助文�?|
| 边框 | `--myst-color-border`, `--myst-color-border-strong` | 分割线、边�?|
...
三变体模式使�?admonition 组件可以统一使用 `bg-myst-{kind}-bg`、`text-myst-{kind}-text`、`border-l-myst-{kind}` 类名，kind 作为变量传入�?
## Tailwind 注册

�?`styles/index.js` �?themeExtensions.colors 中，CSS 变量被注册为 Tailwind 颜色�?
```js
colors: {
  myst: {
...
**关键优势**�?- 只需切换一�?CSS class 即可完成全站点主题切�?- 组件代码不需要任何修改（始终引用相同�?CSS 变量名）
- 过渡动画只需�?CSS 变量上设�?transition
- 用户可以通过浏览器偏好（prefers-color-scheme）或手动切换

ThemeProvider 提供 `theme` �?`setTheme` 接口管理暗色/亮色状态�?
代码高亮也分两套 CSS：`code-highlight-light.css` �?`code-highlight-dark.css`，暗色模式下加载 dark 版本�?
## 品牌定制
...
/* 自定义品牌主�?*/
:root {
  --myst-color-link: #0066cc;        /* 品牌�?*/
  --myst-color-primary: #ff6600;     /* 品牌�?*/
  --myst-color-primary-hover: #ff8533;
}
```

定制方式�?1. **CSS 文件覆盖**：在主题�?`styles/app.css` 中重新定义变�?2. **内联样式**：通过 `<style>` 标签动态注�?3. **Tailwind 配置**：如果需要修�?Tailwind 层面的配置，在主题的 `tailwind.config.js` 中扩�?
## Typography 集成

`@tailwindcss/typography` 插件�?`prose` 类默认颜色被覆盖�?myst CSS 变量�?
```js
typography: (theme) => ({
  DEFAULT: {
    css: {
      ...
      // 移除 code 前后反引�?      'code::before': { content: 'none' },
...
这使�?Tailwind typography 的所有排版样式自动适配 myst 颜色主题，包括暗色模式切换�?
## 与组件库的关�?
CSS 变量是样式系统的**唯一真理�?*（single source of truth）：
- myst-to-react 组件不硬编码颜色值，全部使用 `bg-myst-*`/`text-myst-*` Tailwind �?- themes/book �?themes/article 通过覆盖 CSS 变量实现不同的视觉风�?- jupyter 包的组件（BinderBadge、Outputs 等）同样使用 myst 颜色变量
