...
本分组包含 Python 科学计算与数据科学生态中 6 个核心库的系统化中文教程，全部基于源码深度阅读生成，遵循 [OKF v0.2 规范](https://github.com/awesome-flexloop/awesome-okf)。
...
1. **科学计算基础**：NumPy（ndarray → dtype → ufunc → 广播）
2. **数据分析**：NumPy → pandas（DataFrame → GroupBy → 时间序列）
3. **静态可视化**：matplotlib（Artist → 后端 → pyplot）
4. **交互式可视化**：plotly.py（Figure → Express → 渲染）
5. **Web应用**：plotly.py → Dash（回调 → 组件 → 多页面）
6. **数据存储**：NumPy → PyTables（HDF5 → Table → 压缩索引）
