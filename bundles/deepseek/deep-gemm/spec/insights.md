...
- **DeepEP**（[/deepseek/deep-ep/](/deepseek/deep-ep/)）：DeepSeek 的专家并行通信库，提供 EP（Expert Parallelism）的 all-to-all 通信原语，与 DeepGEMM 的 MegaMoE 协同使用——DeepEP 负责通信，DeepGEMM 负责计算
- **TileLang**（[/deepseek/tile-kernels/](/deepseek/tile-kernels/)）：TileLang 编写的核函数库，`third-party/tilelang_ops/` 包含 SwiGLU+weight 到 FP8 的融合算子，作为 DeepGEMM 核函数的补充
