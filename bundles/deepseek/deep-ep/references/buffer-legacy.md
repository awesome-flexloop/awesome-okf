...
`Buffer` 是 DeepEP V1 的通信缓冲区类，基于 NVSHMEM 构建，支持三种通信模式：节点内高吞吐（NVLink）、节点间高吞吐（RDMA+NVLink）、低延迟（IBGDA）。新代码推荐使用 [ElasticBuffer](/deepseek/deep-ep/references/buffer-elastic)。
...
- [ElasticBuffer API](/deepseek/deep-ep/references/buffer-elastic) — V2 推荐使用的弹性缓冲区
- [Elastic vs Legacy 对比](/deepseek/deep-ep/concepts/elastic-vs-legacy)
- [低延迟模式](/deepseek/deep-ep/concepts/low-latency-mode)
- [事件系统](/deepseek/deep-ep/references/events)
