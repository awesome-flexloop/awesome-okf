...
- `handle`：[EPHandle](/deepseek/deep-ep/references/buffer-elastic#ephandle-类)，路由元数据句柄
- `event`：[EventOverlap](/deepseek/deep-ep/references/events)，事件句柄用于同步
...
在 EP+PP 混合并行中（如 [DualPipe](/deepseek/dual-pipe)），ElasticBuffer 的 PP send/recv API 提供了与 dispatch/combine 共享同一对称内存缓冲区的流水线通信能力，避免额外的内存分配和拷贝。详见 [ElasticBuffer API](/deepseek/deep-ep/references/buffer-elastic#流水线并行pp)。

## 相关参考

- [ElasticBuffer API](/deepseek/deep-ep/references/buffer-elastic) — dispatch/combine 完整参数说明
- [EPHandle 类](/deepseek/deep-ep/references/buffer-elastic#ephandle-类) — 路由元数据详细字段
- [事件系统](/deepseek/deep-ep/references/events) — EventOverlap 同步机制
- [基础 MoE 示例](/deepseek/deep-ep/examples/basic-moe) — 可运行的 dispatch/combine 代码
- [计算-通信重叠示例](/deepseek/deep-ep/examples/event-overlap) — EventOverlap 使用示例
