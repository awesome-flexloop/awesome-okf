...
description: OKF 开放知识格式 v0.2 完整英文规范原文（vendored third-party spec），本 bundle 的权威信源。
tags: [okf, spec, reference, authoritative]
...
本文件收录 OKF（Open Knowledge Format，开放知识格式）v0.2 的完整英文规范原文，来自 Google Cloud Platform knowledge-catalog 仓库。

- **规范版本**：v0.2
- **原始来源**：[GoogleCloudPlatform/knowledge-catalog/okf/SPEC.md](https://github.com/GoogleCloudPlatform/knowledge-catalog/blob/main/okf/SPEC.md)
- **许可协议**：MIT License
- **文件性质**：vendored 第三方规范，英文原文只读，中文翻译与解读见本 bundle 的 concepts/ 目录

本规范定义了 OKF v0.2 的完整格式要求：知识包（bundle）目录结构、概念文档 frontmatter 规范、溯源（sources）/信任（generated/verified）/生命周期（status/stale_after）字段族、交叉链接与路径约定、Actor 身份约定、索引文件与日志文件格式、可认证计算（Attested Computation）概念、合规性要求、版本控制规则，以及相对 v0.1 的变更说明。

本 bundle 中的所有中文概念文档（concepts/）和示例文档（examples/）均以本文件为权威信源进行转译与解读。
