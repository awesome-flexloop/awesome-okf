...
| 概念 | 说明 |
|------|------|
| [智能体范式与ReAct](/datawhale/hello-agents/concepts/agent-paradigms-react) | ReAct、Plan-and-Solve、Reflection三大经典范式的原理与实现 |
| [Agent框架开发](/datawhale/hello-agents/concepts/agent-framework-development) | 从手动实现到框架抽象，AutoGen/AgentScope/CAMEL/LangGraph对比与HelloAgents自建 |
| [记忆系统](/datawhale/hello-agents/concepts/memory-systems) | 四层记忆架构（工作/情景/语义/感知）与RAG检索增强生成 |
| [上下文工程](/datawhale/hello-agents/concepts/context-engineering) | GSSC流水线、上下文腐蚀、JIT检索、长时程任务管理 |
| [通信协议](/datawhale/hello-agents/concepts/communication-protocols) | MCP（Agent-工具）、A2A（Agent-Agent）、ANP（Agent网络）三层协议栈 |
| [多Agent协作](/datawhale/hello-agents/concepts/multi-agent-collaboration) | 对话驱动、角色扮演、图结构工作流等协作模式 |
| [Agentic-RL](/datawhale/hello-agents/concepts/agentic-rl) | 从SFT到GRPO，将LLM作为可学习策略的多步决策优化 |
| [评估方法](/datawhale/hello-agents/concepts/evaluation-methods) | BFCL工具调用评估、GAIA通用能力评估、LLM Judge与数据生成质量评估 |

## 实战示例

- [ReAct范式从零实现](/datawhale/hello-agents/examples/react-implementation)：Thought→Action→Observation循环的完整代码
- [HelloAgents框架架构](/datawhale/hello-agents/examples/helloagents-framework)："万物皆工具"的轻量级框架设计
- [MCP协议集成](/datawhale/hello-agents/examples/mcp-integration)：天气MCP服务器与多Agent文档助手
- [Agentic-RL训练Pipeline](/datawhale/hello-agents/examples/agentic-rl-training)：SFT+GRPO全流程训练代码
