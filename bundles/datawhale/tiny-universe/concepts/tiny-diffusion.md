...
TinyDiffusion 属于"生成模型"方向，与 [TinyLLM](/datawhale/tiny-universe/concepts/tiny-llm)（语言模型）并列，共同构成"从零训练模型"层。它不依赖 RAG 或 Agent 模块，可独立学习。

项目 README 中配有 DDPM 架构图（`./content/TinyDiffusion/fig/ddpm.png`），直观展示前向与反向过程。

## 延伸

- 同属生成模型白盒实现：[TinyLLM](/datawhale/tiny-universe/concepts/tiny-llm)
