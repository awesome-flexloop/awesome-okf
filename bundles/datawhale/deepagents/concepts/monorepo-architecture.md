...
- [核心SDK与三层架构](/datawhale/deepagents/concepts/core-sdk) 是 monorepo 中 `deepagents` 包的内部设计。
- [Code终端编码Agent](/datawhale/deepagents/concepts/code-module) 精确钉住核心 SDK 版本，是最大的消费者。
- [CLI部署工具](/datawhale/deepagents/concepts/cli-toolchain) 和 [ACP协议集成](/datawhale/deepagents/concepts/acp-protocol) 是横向扩展包。
- [Evals评估套件](/datawhale/deepagents/concepts/evals-suite) 验证 SDK 和 Code 的行为质量。
- [Talon运行时宿主](/datawhale/deepagents/concepts/talon-runtime) 依赖 deepagents 和 deepagents-code。
